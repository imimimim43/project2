...
- **삽입 정렬**로 정렬, 역순, 랜덤 배열 값을 정렬한 결과, 위 그래프가 그려졌는데 앞서 설명했던 정렬들과 같이 입력 값이 2^12으로 갈수록 역순, 랜덤, 정렬 순으로 정렬 시간이 점점 길어지는 것을 알 수 있다. 그러나 다른 정렬들보다 역순 배열 값을 정렬하는 시간이 조금 더 긴 것을 알 수 있다.
